...
<cfoutput><form novalidate="novalidate" action="index.cfm?fuseaction=cPrivateUsers.update&userid=#URLEncodedFormat(attributes.userid)#&routeid=#attributes.routeid#&siteid=#URLEncodedFormat(attributes.siteid)#" method="post" enctype="multipart/form-data" name="form1" onsubmit="return validate(this);"  autocomplete="off" >
	...
	<cfif len(request.userBean.getUsername())>
		...
		<cfif strikes.isBlocked()>
			...
			<a href="?fuseaction=cPrivateUsers.edituser&userid=#URLEncodedFormat(attributes.userid)#&type=2&siteid=#URLEncodedFormat(attributes.siteid)#&removeBlock">[#application.rbFactory.getKeyValue(session.rb,'user.remove')#]</a>
			</p>
		</cfif>
	</cfif>
...
<div id="tabAddressinformation">
		...
		<dt>#application.rbFactory.getKeyValue(session.rb,'user.email')#</dt>
		<dd><input id="addressEmail" name="addressEmail" validate="email" message="#application.rbFactory.getKeyValue(session.rb,'user.emailvalidate')#" type="text" value="#HTMLEditFormat(attributes.addressEmail)#" class="text"></dd>
		...
		<dd><ul>
			<cfloop collection="#application.settingsManager.getSites()#" item="site">
				<cfif application.settingsManager.getSite(site).getPrivateUserPoolID() eq application.settingsManager.getSite(attributes.siteid).getPrivateUserPoolID()>
					<li><input type="checkbox" class="checkbox" name="ContactForm" value="#site#" <cfif listfind(request.userBean.getcontactform(),site)>Checked</cfif>>#application.settingsManager.getSite(site).getSite()#</li>
				</cfif>
			</cfloop></ul>
		</dd>
